...
# REMOVED call_vision_api FUNCTION
...
def main():
    ...
    if (functionality == 'Chat'):
        ...
        if selected_prompt_label == 'Custom':
            # If the user selects "Custom", show a text area and submit button for custom prompt
            with st.sidebar.form(key='custom_prompt_form'):
                custom_prompt = st.text_area("Enter your custom prompt:")
                custom_prompt_submit = st.form_submit_button("Submit")
            if custom_prompt_submit:
